...
std::vector<GLuint> Model::getIndices(json accessor)
{
	...
	if (!accessor.contains("count") || accessor["count"].is_null()) {
		std::cerr << "[getIndices] Error: 'count' is missing or null\n";
		return indices;
	}
	if (!accessor.contains("componentType") || accessor["componentType"].is_null()) {
		std::cerr << "[getIndices] Error: 'componentType' is missing or null\n";
		return indices;
	}
	if (!accessor.contains("bufferView") || accessor["bufferView"].is_null()) {
		std::cerr << "[getIndices] Error: 'bufferView' is missing or null\n";
		return indices;
	}
	...
	if (!JSON.contains("bufferViews") || buffViewInd >= JSON["bufferViews"].size()) {
		std::cerr << "[getIndices] Error: Invalid bufferView index: " << buffViewInd << "\n";
		return indices;
	}
	...
	{
		std::cerr << "[getIndices] Error: Unknown component type: " << componentType << "\n";
	}
